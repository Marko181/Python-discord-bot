...
class VoiceCog(commands.GroupCog, name="muzika"):
    ...
    async def queue(self, interaction: discord.Interaction, file_name: str):
        ...
        if not is_playing:
            ...
            await interaction.followup.send(f"Enqueued '{file_name}' at position {len(queue)}.", ephemeral=False)
    ...
    @commands.command(name="stoj", description="Stop playback and clear the queue")
    async def stop(self, interaction: discord.Interaction):
        await interaction.response.defer(ephemeral=False)  # <- This prevents timeout

        vc = discord.utils.get(self.bot.voice_clients, guild=interaction.guild)
        if not vc or not vc.is_playing():
            return await interaction.followup.send("No audio is playing right now.", ephemeral=True)
